...
from transformers.models.bert.configuration_bert import PretrainedConfig
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mapping_layer = nn.Linear(self.embedding_size, config.hidden_size)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        if self.config.hidden_size != self.embedding_size:
            embeddings = self.mapping_layer(embeddings)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # 有key_size就用key_size，增大q与k的size可以提高效果，https://arxiv.org/pdf/2002.07028.pdf
        self.all_head_size = config.key_size or self.num_attention_heads * self.attention_head_size

        self.query = nn.Linear(config.hidden_size, self.all_head_size)
        self.key = nn.Linear(config.hidden_size, self.all_head_size)
        self.value = nn.Linear(config.hidden_size, self.num_attention_heads * self.attention_head_size)
...
class ReadingClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size * 6, 1)
        self.act = nn.Sigmoid()

        self.content_dense = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def forward(
        ...
        out = torch.cat([pooled_output, content, cat1, cat2, cat3, cat4], dim=-1)
